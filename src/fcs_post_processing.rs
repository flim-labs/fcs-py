    ...
    let g2_correlations: Vec<_> = correlations
        ...
        .filter_map(|&correlation| {
            ...
            let intensity2_data = intensities
                ...
                .data
                .clone();
            let g2_values = calculate_correlation(&intensity1_data, &intensity2_data, &lag_index);
...
fn calculate_lag_index(taus_number: usize) -> Vec<i32> {
    ...
    let lag_index: Vec<i32> = tau_ind.iter().map(|&x| x as i32).collect();
    let mut lag_index_with_zero = lag_index;
    lag_index_with_zero.insert(0, 0);
    lag_index_with_zero
}
