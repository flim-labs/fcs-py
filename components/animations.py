...
class VibrantAnimation:
    ...
    def start(self, amplitude=10, duration=50):
        ...
        self.widget.setStyleSheet(        
                    f"QLabel {{ color : {self.start_color}; font-size: 30px; font-weight: bold; background-color: {self.bg_color};}}"
                )         
    ...
    def stop(self):
        if self.animation.state() == QAbstractAnimation.State.Running:
            self.widget.setStyleSheet(        
                        f"QLabel {{ color : {self.stop_color}; font-size: 30px; font-weight: bold; background-color: {self.bg_color};}}"
                    )              
            self.animation.stop()
