...
class FCSPostProcessing:
    ...
    def start_fcs_post_processing(app, intensities_input, active_correlations, bin_width_us, taus_number, acquisition_time_us):
        ...
        for index, res in enumerate(gt_results[1]):
            correlation = res[0]
            gt_values = res[1]    
            if tuple(correlation) in gt_plot_to_show:
...
class FCSPostProcessingPlot:
    ...
    def generate_chart(correlation, index, app, lag_index, gt_values):
        ...
        gt_widget.setLabel('left', 'G(τ)')
        gt_widget.setLabel('bottom', 'τ',)
        ...
        gt_widget.plotItem.getAxis('bottom').setLogMode(False)
        gt_widget.plotItem.getAxis('left').setLogMode(False)

        gt_widget.setBackground("#0E0E0E")
        gt_widget.setStyleSheet("border: 1px solid #3b3b3b")
        app.gt_lines.append(intensity_plot)
        app.layouts[GT_PLOTS_GRID].addWidget(gt_widget)
